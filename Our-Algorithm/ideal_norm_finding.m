...
walk_length:=8;
ideal_norm_seq:=[];
initial_ideal_norm_seq:=[];
time for i in [1..100] do
	j:=j+1;
	ideal_for_random_walk,exponent_walk_ideal:=Explode(products_of_ideal_and_exponent(2,bound_on_exp));PrintFile("ideal_now",ideal_for_random_walk:Overwrite:=true); 	Append(~initial_ideal_norm_seq,Norm(ideal_for_random_walk));
	...
	while(length_now le walk_length)do
		...
		Append(~ideal_norm_seq,Norm(ideal_for_random_walk));
		...
		if((itr mod 10) eq 1) then
			printf "\n itr=%o",itr;
			printf "\n ideal_norm_seq=%o",ideal_norm_seq;
		end if;
		if(abs eq 0) then
			printf "\n breaking as abs = 0";
			break;	
...
printf "\n after everything ideal_norm_seq=%o",ideal_norm_seq;
printf "\n #precomputed_table eq (R*q)=%o", #precomputed_table eq (R*q);
printf "\n fb_bound_B=%o, bound_k=%o, R=%o, q=%o, r=%o, table_size=%o",fb_bound_B,bound_k,R,q,r,#precomputed_table;

m:=Max(ideal_norm_seq);
printf "\n max norm log with respect 2 considering walk=%o",Log(2,m);
m:=Max(initial_ideal_norm_seq);
printf "\n max norm log with respect 2 taking only initial ideals of the walk=%o",Log(2,m);
